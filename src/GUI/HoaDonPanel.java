...
public class HoaDonPanel extends JPanel {
	...
	private String[] hoaDonJTableColumns = {"ID Hóa đơn", "Ngày tạo", "idNV", "Tên nhân viên", "Tổng tiền"};
	private String[] ctHoaDonJTableColumns = {"idSP", "Tên sản phẩm", "Số lượng", "Giá lúc đặt", "Thành tiền"};
	private Map<Integer, String> nhanVienMap; // Map để lưu idNV -> hoten
	...
	private void setupComboIdNV() {
		...
		for (TaiKhoanDTO tk : nhanVienList) {
			comboIdNV.addItem(new ComboItem(tk.getIdTK(), tk.getIdTK() + " - " + tk.getHoten()));
		}
	}
	...
	private void loadCTHoaDon(int idHD) {
		...
		String tenNV = nhanVienMap.getOrDefault(selectedHD.getIdTK(), "Không rõ");
		...
		lblHoaDonInfo.setText(
				...
				" Nhân viên tạo hóa đơn: " + selectedHD.getIdTK() + " - " + tenNV + " | " +
				" Tổng tiền: " + tongTien
		);
	...
	private void loadSanPhamMap() {
		List<SanPhamDTO> sanPhamList = sanPhamBUS.getAllSP();
	...
	private void loadHoaDonList(List<HoaDonDTO> hoaDonList) {
		...
		for (HoaDonDTO hd : hoaDonList) {
			...
			int tongTien = ctList.stream()
					...
					.sum();
			String tenNV = nhanVienMap.getOrDefault(hd.getIdTK(), "Không rõ");
