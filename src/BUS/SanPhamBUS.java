	...
	public List<SanPhamDTO> search(String keyWord, DanhMucDTO danhMuc){
		...
		if(keyWord.isEmpty()){
			...
			if(danhMuc.getIdDM() == 0) {
				result = sanPhamDao.getAllActive();
			} else {
				result = sanPhamDao.searchByCategory(danhMuc.getIdDM());
			}
		} else{
			// kiếm theo keyword và danh mục
			if(danhMuc.getIdDM() == 0) {
				result = sanPhamDao.searchByKeyWord(keyWord);
			} else {
				result = sanPhamDao.search(keyWord, danhMuc.getIdDM());
			}
		}
		return result;
	}
